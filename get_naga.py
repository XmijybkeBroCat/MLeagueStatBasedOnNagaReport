...
def fetch_naga_report_data(naga_url: str, filename: str, origin: list, similarity: list) -> None:
    """
    ...
    try:
        ...
        data = resp.json()
        output = {'Players': data['player_info']['name'], 'Frames': data['custom_haihu'], 'Origin': origin}
        if similarity is not None:
            output['Similarity'] = similarity
        json.dump(output, open(filename, 'w', encoding='UTF-8'), ensure_ascii=False, indent=2)
...
if __name__ == "__main__":
    ...
    fetch_naga_report_data(test_url, 'regular/r1.json', [40000, 30000, 20000, 10000])